...
class Solution:
    def __init__(self, cluster_centers, label, data):
        ...
        self.label = label
        self.ss_distance = np.sum([distance.pdist([cluster_centers[i], data[label == i][j]], 'sqeuclidean') for i in range(len(cluster_centers)) for j in range(len(data[label == i]))])


class KMeansDE(BaseEstimator, ClusterMixin):

    def __init__(self, n_clusters=2, max_iter=5000, tol=1e-3,
                 verbose=0, population_lenght=150, kmeans_max_iter=50000):
    ...
    def _prepareDataset(self, data):
        ...
        if self.verbose:
            print("dati", data.head())

        scaler = StandardScaler()
        scaled_array = scaler.fit_transform(data)
        scaled_dataframe = pd.DataFrame(scaled_array, columns=data.columns)
        if self.verbose:
            print("dati scalati", scaled_dataframe.head())
    ...
    def _initializeS(self):
        ...
        for i in range(self.population_lenght):
            kmeans_model = KMeans(n_clusters=self.n_clusters, init='random', max_iter=1,
                                  n_init=1, random_state=random.randint(1, 100000000))  # preparo l'algoritmo di kmeans
    ...
    def _stopping_criterion(self, consecutive_else, pick_success):
        if not pick_success:
            if self.verbose:
                print("Tutta la popolazione è uguale")
            return False

        if self.verbose:
            print("consec iter ", consecutive_else)

        if consecutive_else >= self.max_iter:
            if self.verbose:
                print("Max Iter raggiunte")
            return False
        ...
        if len(population_unique) < len(self.population) and self.verbose:
            print("Qualcosa è uguale", len(self.population) - len(population_unique))

        for k in range(len(population_unique)):

            sum += np.sum([np.abs(population_unique[k].ss_distance - population_unique[x].ss_distance) for x in range(k + 1, len(population_unique))])

        if self.verbose:
            print("toll ", self.tol, "sum ", sum)

        if sum <= self.tol:
            if self.verbose:
                print('Quella roba con la tolleranza')
            return False
        else:
            return True
    ...
    def _best_solution(self):
        ...
        for i in range(self.population_lenght - 1):
            ...
            if trial < best:
                ...
                best_index = i
        if self.verbose:
            print("la migliore soluzione è la: " + str(best_index + 1) + "-esima componente di S, ",
...
if __name__ == '__main__':
    bupa_data = np.loadtxt("bupa.data", delimiter=',')
    ...
    km = KMeansDE(n_clusters=2, verbose=1)
